# Guion de defensa — TFG EPS0270 · Dermapixel · **VERSIÓN RECORTADA**

> **Copia de trabajo.** La versión completa ("director's cut") está en `GUION_DEFENSA_TFG.md`. Aquí se han recortado los slides "gordos" (2, 4, 9, 10, 11, 16, 18, 19, 21) y el 20. **Estado actual: Parte A ≈ 24,8 min · prototipo ≈ 4,8 · demo ≈ 5,5 · cierre ≈ 1,8 → TOTAL ≈ 37 min.** Para bajar a 30 quedan las palancas del final (fusionar 12+13 y 14+15, y capar la demo a 4-5 min).
...
## Slide 2 — El problema clínico (65 s)
...
## Slide 4 — Tres retos abiertos, la pregunta y el anuncio (75 s) · ★ una de las slides clave
...
>
> *(pausa)* Y hay una idea que me gustaría que recordaran. *(pausa)* Después de evaluar los modelos de forma independiente, construir un recurso clínico en español y llevarlo a un prototipo, la conclusión principal no es cuál es el mejor modelo: es que, en IA médica, **el principal factor que determina el rendimiento no es únicamente la arquitectura, sino la calidad de los datos sobre los que se entrena y se evalúa**.
*(★ Anuncio del mensaje — conversacional, con pausas, mirando al tribunal. Se reformula en la 21 y culmina en la 29.)*
...
## Slide 9 — Un lenguaje común: la ontología L1/L2/L3 (65 s) · ★ contribución propia
...
## Slide 16 — Resultado 6 · El texto también importa (75 s)
...
## Slide 18 — Resultado 8 · Equidad y validación estadística (75 s)
**VISUAL:** Dos mitades.
...
## Slide 20 — De evaluar a crear: tres contribuciones propias (95 s) · ★ slide clave · aquí se explica LoRA (1.ª vez)
**VISUAL:** Tres bloques.
...
>
> La primera, **DermapixelAI 1.0**. Casi todos los recursos disponibles estaban en inglés y en dermatoscopia; faltaba un recurso clínico en español con texto real. Lo construimos a partir del archivo de la Dra. Taberner: 672 casos, 1.089 imágenes y textos clínicos reales, mediana de 223 palabras. Y subrayo algo: **no es un dataset de imágenes, es un dataset multimodal** —imagen, texto y caso. Documentado, auditado frente a Derm1M —cero solapamiento— y liberado bajo licencia abierta.
>
> La segunda, **Dermapixel R0**, la adaptación de PanDerm al castellano. Usamos LoRA: entre congelar el modelo o reentrenarlo entero, es el punto intermedio —congela el modelo y solo añade unas matrices pequeñas. Es como si ya hubiera estudiado toda la dermatología y, en lugar de reescribir el libro, le añadiéramos unas páginas con lo específico de la tarea. Entrenamos solo **524.000 parámetros, el 0,17%** —y aun así R0 iguala a DermLIP v2 en la clasificación clínica en español. Esto **valida una conclusión de antes**: con pocos datos, las adaptaciones eficientes ganan al fine-tuning completo.
>
> La tercera no es un resultado positivo, sino uno **igualmente útil**: la recuperación por texto en español es viable, pero el volumen de datos aún no basta para generalizar. No es un fracaso, es una **hipótesis validada en parte**: la arquitectura funciona; el cuello de botella ya no es el método, es el dato en español. Marca el siguiente paso —el banco del hospital—; de hecho ya lo hemos empezado a comprobar, **fuera del alcance de esta memoria**: al ampliar el dataset con material cedido por el Dr. Llambrich, la **clasificación mejora**, pero la **búsqueda por texto todavía no**; justo lo previsto, el límite es el dato.
>
> En conjunto, estas tres contribuciones **transforman un benchmark en una plataforma**: un recurso abierto en español, un modelo adaptado al dominio clínico, y una evolución clara hacia un sistema asistencial.
...
## Slide 21 — Conclusiones de la investigación · tres descubrimientos (75 s)
