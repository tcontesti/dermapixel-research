...
declarados y métricas esperables. Todas descansan sobre las
contribuciones reproducibles enumeradas en
sección~\ref{sec:contribuciones}.

Conviene distinguir con precisión dos registros dentro de esta
sección. Algunas subsecciones recogen trabajo ya \emph{ejecutado} y
reportado en el capítulo~\ref{cap:resultados} ---la cobertura
experimental sobre DermapixelAI~1.0, el eje Derm7pt + Sparse
Autoencoders y la selección de arquitectura de segmentación---, que se
presenta aquí como base consolidada y no como agenda pendiente. Las
restantes ---el escalado con el banco hospitalario del HUSLL, la
validación clínica prospectiva, el aprendizaje activo, la IA agéntica
y el reentrenamiento multilingüe de la rama lingüística--- son agenda
futura genuina, condicionada a recursos o aprobaciones específicas y
no ejecutada en este trabajo.
...
aportación más original: el primer modelo fundacional dermatológico
adaptado al español del que se tiene constancia a la fecha de cierre.
La denominación se emplea en sentido preciso: Dermapixel R0 no es un
preentrenamiento fundacional a gran escala ---como PanDerm o
Derm1M---, sino la adaptación de un modelo fundacional preexistente
(PanDerm Large) al registro clínico en castellano; el calificativo
\emph{primero} se acota a esa categoría ---adaptación con pesos
abiertos, texto clínico narrativo y registro en español--- y no a una
prioridad histórica absoluta sobre cualquier recurso dermatológico
hispanohablante. Su relevancia no reside en una métrica de máximo
rendimiento ---es,
por diseño, una versión base (R0) entrenada sobre un dataset reducido,
un punto de partida y no un modelo terminado---, sino en lo que
demuestra: que un modelo dermatológico especializado en castellano
puede construirse sobre un modelo fundacional liberado mediante
técnicas de adaptación eficiente, con un presupuesto de cómputo y de
datos al alcance de una institución hospitalaria y un grupo académico.
El preentrenamiento de los modelos de base exige los grandes datasets
y el cómputo de grupos de investigación especializados; este trabajo
evidencia que la especialización posterior ---lingüística y
clínica--- no comparte esa barrera de entrada.

Dermapixel R0 se inicializa desde el codificador PanDerm Large, se
adapta sobre el dataset DermapixelAI~1.0 y se articula en dos ramas
complementarias.
...
En conjunto, Dermapixel R0 traslada al dominio dermatológico en
español una conclusión de alcance más amplio: la frontera de la
especialización ya no la marca el acceso a grandes infraestructuras de
preentrenamiento, sino la disponibilidad de datos clínicos bien
curados y de un protocolo de adaptación adecuado. La rama supervisada
L2 ya ejecutada (sección~\ref{sec:dermapixel-spanderm-v0}) demuestra
el principio sobre la cardinalidad clínica intermedia, y la cabeza L3
directa ---condicionada al banco hospitalario HUSLL--- define la ruta
de escalado natural. Como semilla reproducible y honestamente acotada,
Dermapixel R0 deja establecida una línea de continuación que otras
instituciones con archivos clínicos en su propia lengua podrían
replicar sobre la misma receta de adaptación eficiente.

\subsection{Derm7pt + Sparse Autoencoders: ground-truth conceptual
para interpretabilidad}
\label{sec:linea-derm7pt-sae}

Las anotaciones \emph{Seven-Point Checklist} de Derm7pt (siete
criterios diagnósticos por imagen) constituyen ground-truth conceptual
no explotada por la literatura previa sobre Sparse Autoencoders en
dermatología. Los cuatro experimentos previstos sobre este eje (E1,
correlación SAE-Derm7pt; E2, \emph{Concept Bottleneck Model}; E3,
cruce con los conceptos del Grupo~A; E4, \emph{fine-tuning} multitarea
\emph{concept-aware}) se han \emph{ejecutado} y se reportan en la
sección~\ref{sec:sae-derm7pt} y en el Anexo~\ref{cap:anexof}; no
constituyen, por tanto, trabajo pendiente, sino contribución ya
realizada. Su resultado principal es que la base \emph{sparse}
aprendida sobre Derm1M alinea \emph{features} individuales con la
taxonomía dermatoscópica sin entrenamiento específico (AUROC efectivo
hasta $0{,}926$ sobre vasos en horquilla), y que el \emph{fine-tuning}
multitarea con LoRA produce inferencia simultánea de melanoma y los
siete criterios en una sola pasada (cabeza melanoma AUROC $0{,}891$),
consolidando el módulo de interpretabilidad del prototipo
(Anexo~\ref{cap:anexoh}) sin pipelines en cascada.

La línea genuinamente abierta es la extensión del diccionario
conceptual a los $15$ conceptos clínicos adicionales propuestos por la
Dra.~R.~Taberner (Anexo~\ref{cap:anexof}) ---no contemplados por
SkinCon ni por el \emph{Seven-Point Checklist}---, condicionada al
muestreo y anotación de aproximadamente $200$ imágenes por concepto.
...
(sección~\ref{sec:spanderm-clip}) mostró que ampliar el dataset de
entrenamiento con escala genérica externa (v5) no rellena la cola
larga de L3 ---$327$ clases con $\leq 5$ imágenes en el vocabulario
ontológico--- y erosiona la alineación cross-modal sobre el registro
castellano, mientras que la iteración v4 ya alcanza el umbral
\emph{a priori} de \emph{accuracy}@1 $\sim 0{,}25$. La palanca correcta
para extender la cobertura no es, por tanto, más datos genéricos, sino
aprendizaje activo dirigido \emph{in-domain}: identificar las clases
L3 con peor cobertura en el subconjunto castellano e invertir en ellas
anotación experta, con los candidatos generados por el prototipo
DermApIxel sobre cohortes del HUSLL bajo revisión de la
Dra.~R.~Taberner.

El banco hospitalario HUSLL ($\sim 22\,000$ estudios tras aprobación
CEIC, sección~\ref{sec:linea-husll}) es el recurso natural: refleja la
distribución diagnóstica real del servicio, está alineado con el
registro castellano del archivo Dermapixel y permite tanto el llenado
dirigido de la cola larga como la eventual activación de la cabeza L3
directa al superar las $\sim 30$ imágenes por clase
(sección~\ref{sec:linea-spanderm}). Tras alcanzar v4 el umbral
operativo, esta línea se replantea como ruta de generalización hacia
cobertura clínica completa, no hacia el \emph{target}. La ablación del
Anexo~\ref{cap:anexoi} la refuerza: la palanca pendiente es anotación
experta \emph{in-domain}, no escala genérica de datos.
...
pauta de selección reproducible: la generación SAM2 domina sobre SAM
v1 con independencia del preentrenamiento de dominio, el factor
determinante del rendimiento es la caja-\emph{prompt} y no el
segmentador, y MedSAM2-tiny (Dice $0{,}9556$, $26$\,ms) se identifica
como candidata de referencia para despliegues con restricciones de
cómputo, con la cifra estable bajo validación cruzada y próxima al
techo de acuerdo inter-anotador. La línea abierta se reduce, por
tanto, a dos elementos no resueltos en este trabajo: la evaluación con
un detector de lesión extremo a extremo que sustituya la caja manual y
la validación prospectiva sobre adquisición clínica no dermatoscópica.
...
datasets de preentrenamiento del ecosistema dermatológico (SkinGPT-4,
MONET, MM-Skin, SkinFM, entre otros). La línea abierta consiste en
aplicar el mismo procedimiento sobre los \emph{splits} de evaluación
canónicos y publicar las cardinalidades de intersección, con su
script auditable, como recurso de referencia que permita recalibrar
las cifras zero-shot publicadas sobre esos datasets.

\subsection{Recuperación visual densa como sistema de apoyo}
\label{sec:linea-rag}

La recuperación visual densa basada en FAISS sobre los
\emph{embeddings} de Derm1M ($\sim 421\,000$ imágenes preindexadas)
ya opera como componente del prototipo DermApIxel
(sección~\ref{sec:dermapixel-m4} del Anexo~\ref{cap:anexoh}) con
latencia inferior a $150$\,ms por consulta. La línea abierta es
evaluar su valor clínico mediante un estudio mixto sobre cohorte
HUSLL ---\emph{precision@k} sobre L1/L2 y valoración subjetiva de la
utilidad de los casos recuperados como referencia diferencial---, con
la integración en la historia clínica electrónica como prerrequisito
técnico pendiente.
