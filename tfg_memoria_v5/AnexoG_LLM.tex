...
multimodales generalistas presentada en sección~\ref{sec:llm-results} del
...
asimétricas reportadas en sección~\ref{sec:disc-llm}. La
...
(sección~\ref{sec:ft}). La diferencia metodológica sugiere que el
...
(sección~\ref{sec:dermapixel-m6}) opera sobre DermLIP v2 con
envoltorio automático del \emph{prompt}. Esta sección documenta
cuantitativamente el fenómeno que motiva el envoltorio, resumido en
la tabla~\ref{tab:anexoh-prompt-curva}.
...
sobre \emph{captions} del dataset Derm1M con estructura
...
envoltorio \emph{``dermoscopy image of \{term\}''} cuando la entrada
...
escribe descripciones completas propias. La extensión de este barrido sistemático de
longitud de \emph{prompt} a los demás datasets de evaluación
...
patrones cualitativos identificados en sección~\ref{sec:disc-llm} del
...
\emph{prompt} clínico descrito en sección~\ref{sec:anexoh-prompt}.
