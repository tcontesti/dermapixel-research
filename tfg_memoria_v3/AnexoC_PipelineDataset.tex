...
\section{Cardinalidades del corpus y splits experimentales}
\label{sec:anexo-cardinalidades}

A lo largo del trabajo coexisten tres particiones distintas del
corpus, todas derivadas del mismo pipeline pero filtradas con
...
\centering
\small
\caption{Cardinalidades del corpus DermapixelAI~1.0 según
configuración. Los splits son \emph{case-aware} (ningún caso se
reparte entre splits). El test del estudio principal sobre el
corpus de producción es deliberadamente reducido para reservar
...
SpanDerm-CLIP (Sec.~\ref{sec:spanderm-clip}) & 1\,062 & 653 & 854 & 107 / 101 & Experimento dual-encoder con \emph{manifest} SHA-256 propio \\
\hline
\end{tabular}
\end{table}
...
los splits SpanDerm-CLIP de $854/107/101$ son reproducibles a
...
La distribución por L1 muestra el predominio de la patología
inflamatoria, seguida por las patologías tumoral e infecciosa con
pesos similares y un porcentaje residual de genodermatosis.
...
caracterización del corpus efectivo. Patología inflamatoria
concentra la mitad del corpus, mientras que Genodermatosis ocupa la
...
representadas en el corpus, ordenadas por número de imágenes.}
\label{fig:eda-l2}
\end{figure}
...
mayor número de imágenes en el corpus. Estos diez diagnósticos
concentran $234$ imágenes, equivalentes al $22{,}0$\,\% del corpus
filtrado.

\begin{table}[H]
\centering
\small
\caption{Top-10 diagnósticos L3 más frecuentes en el corpus
DermapixelAI 1.0.}
...
La distribución por L3 presenta una cola larga pronunciada. La
tabla~\ref{tab:anexo-cola-larga} cuantifica el reparto de las 250
clases L3 efectivamente representadas en el corpus según el número
de imágenes asociadas a cada clase.

\begin{table}[H]
\centering
\footnotesize
\caption{Cola larga L3 sobre las 250 clases efectivas del corpus
DermapixelAI 1.0.}
...
cuenta con una única imagen. Esta característica condiciona la
...
ontology} y L1 asignada). Las cifras sobre el corpus completo
...
(97,93\,\%), no la tercera. La figura~\ref{fig:eda-calidad}
...
con etiqueta L3 mapeada en la ontología ($97{,}93$\,\%) y la
...
Las imágenes del corpus DermapixelAI 1.0 se distribuyen en un rango
...
imágenes incorporadas al corpus. El año de mayor aportación es 2017
...
corpus DermapixelAI 1.0. Rango $2011$--$2026$; año pico $2017$
con $87$ imágenes.}
