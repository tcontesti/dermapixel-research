...
secci\'on~\ref{sec:dermapixel-m11}. El prototipo no constituye
objeto experimental del estudio principal del documento
(secci\'on~\ref{sec:alcance}), pero materializa la integración
...
La arquitectura resultante (secci\'on~\ref{sec:dermapixel-arq}) implementa
...
(IP $192.168.1.37$), aloja la API REST, la base de datos, el
proxy inverso y la interfaz web. El segundo nodo, NVIDIA DGX Spark
con chip Grace Hopper GB10 (IP $192.168.1.36$), aloja el
...
La base de datos contiene once tablas relacionales que documentan
...
descrita en secci\'on~\ref{sec:ft}. La inferencia emplea \emph{test-time
...
secci\'on~\ref{sec:ensemble-results}). La salida es un vector de siete
...
M2 es SAM2.1-Large adaptado mediante LoRA conforme a la
configuración descrita en secci\'on~\ref{sec:segmentacion}. La inferencia
...
$50\,454$ imágenes de siete datasets (secci\'on~\ref{sec:disc-equidad}
...
consulta sobre el equipo descrito en secci\'on~\ref{sec:infraestructura}.
...
(secci\'on~\ref{sec:dermapixel-m5}), donde los hallazgos de M1--M4 se
...
documentado en secci\'on~\ref{sec:disc-llm} y atribuible a la diferencia
...
secci\'on~\ref{sec:zs} usando DermLIP v2 (ViT-B/16 + PubMedBERT) con los
...
(secci\'on~\ref{sec:disc-zs}) es la sensibilidad extrema de DermLIP v2 a
...
continuación inmediata en secci\'on~\ref{sec:linea-prospectivo}. Los
...
protocolos estándar (secci\'on~\ref{sec:protocolos}). El prototipo
...
clínica prospectiva descrita en secci\'on~\ref{sec:linea-prospectivo}.

Tres aportaciones técnicas merecen mención específica. Primero,
la verificación de que seis modelos heterogéneos
(PanDerm Large FT, SAM2.1-Large+LoRA, SAE, FAISS sobre Derm1M,
...
en M5, que sostiene la afirmación de secci\'on~\ref{sec:disc-llm} sobre la
...
(secci\'on~\ref{sec:disc-zs}), cuya mitigación mediante \emph{wrapping}
...
de continuación inmediatas en la secci\'on~\ref{sec:lineas-abiertas} del
...
secci\'on~\ref{sec:dermapixel-discusion}: el clasificador cerrado
...
\subsection{M9 -- SpanDerm v0, clasificador L2 castellano
supervisado}
\label{sec:dermapixel-m9}

M9 es la implementación operativa del protocolo SpanDerm v0
descrito en secci\'on~\ref{sec:dermapixel-spanderm-v0}. Cabeza
...
del corpus. El \emph{checkpoint} \emph{slim} conserva únicamente
...
respecto al sistema SpanDerm-CLIP descrito en
secci\'on~\ref{sec:spanderm-clip} es operacional: aquí la cabeza
...
secci\'on~\ref{sec:sae-d7pt-e4} sobre el corpus
...
(secci\'on~\ref{sec:dermapixel-m4}) sobre Derm1M en inglés: donde
...
\begin{tabular}{lrrrr}
\hline
Nivel & M1 (HAM FT) & M7 (\emph{merged43}+TTA) & M9 (SpanDerm v0) & M4-bis (RAG ES) \\
\hline
L1   & $1{,}0$           & $1{,}0$           & $0{,}5$           & $0{,}3$           \\
