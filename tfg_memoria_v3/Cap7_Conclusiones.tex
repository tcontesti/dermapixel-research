...
La pregunta formulada en secci\'on~\ref{sec:pregunta} se respondió mediante
...
ajuste fino supervisado, segmentación binaria con LoRA y
...
clases de malignidad, con valores entre $+0{,}124$\,pp BAcc
(BiomedCLIP) y $+0{,}306$\,pp (DINOv2 ViT-L/14). La paradoja
...
$+0{,}217$\,pp); DermLIP v2 ofrece la mejor BAcc global
($0{,}721$) con \emph{gap} superior ($+0{,}281$\,pp).
...
\item \textbf{Benchmark homogéneo de catorce modelos sobre doce
datasets.} El estudio comparativo aplica cuatro protocolos
principales bajo configuración estándar
(secci\'on~\ref{sec:protocolos}) sobre PanDerm Base y Large, DINOv2,
ConvNeXt-Large, EfficientNetV2-Large, tres variantes de DermLIP,
BiomedCLIP, SigLIP-Large, SAM2.1+LoRA, GPT-4o, Gemini~2.5~Pro,
MedGemma~4B Vision y~27B Text, y BLIP-2. Las cifras se reportan
...
externos contrastados, con identificación de tres conjuntos
afectados (Dermnet, HIBA, MSKCC) y declaración de \emph{caveat}
explícito sobre las cifras zero-shot de la familia DermLIP en
estos datasets.
\end{enumerate}
...
mediante LoRA.} Configuración con \emph{rank} $r = 8$ sobre el
\emph{mask\_decoder} y \emph{promptable encoder}, congelando el
...
\paragraph{LoRA sobre encoders promptables preentrenados a gran
escala.}
La combinación SAM2.1-Large preentrenado más adaptación LoRA con
\emph{rank} bajo sobre el decodificador de máscaras supera al
\emph{fine-tuning} completo de baselines dermatológicos
específicos, con incorporación de aproximadamente $4{,}5$\,M
...
de equidad.}
La paradoja BiomedCLyu---\emph{gap}~I-VI mínimo ($+0{,}124$\,pp)
asociado a la peor BAcc global ($0{,}590$)--- demuestra que la
...
Segmentación con SAM2.1-Large adaptado mediante LoRA cuando la
tarea requiera localización de lesión. Ensemble OR \emph{top-3}
...
secci\'on~\ref{sec:contribuciones}.
...
no explorado sobre el corpus en castellano incluye un conjunto de
...
la secci\'on~\ref{sec:ft-results}. La presente sección documenta de
...
\hline
Ajuste fino completo del encoder        & Supervisado            & $\sim 5$ min Spark (3 niveles) & $+18{,}2$\,pp BAcc L1 sobre LP CV; $-4{,}0$\,pp BAcc L2 frente a SpanDerm v0 LoRA; sin mejora sobre LP en L3 & \textbf{Ejecutado} (L1$+$L2$+$L3), secci\'on~\ref{sec:dermapixel-ft} \\
LoRA $r=16$ (SpanDerm v0)               & PEFT                   & $\sim 12$ min Spark (3 seeds) & $+19{,}5$\,pp BAcc L2 sobre LP CV; AUROC L2 $0{,}855$ empata con DermLIP individual & \textbf{Ejecutado}, secci\'on~\ref{sec:dermapixel-spanderm-v0} \\
\emph{Test-time augmentation}           & Inferencia             & $5$--$10$ min              & $+3$ a $+22$\,pp Acc@3 (cf.\ secci\'on~\ref{sec:ensemble-results}) & \textbf{Probado}, secci\'on~\ref{sec:dermapixel-tta} \\
Ensemble Base$+$Large$+$DermLIP         & Inferencia             & Trivial                    & $+6{,}7$\,pp AUROC L2 con DermLIP individual; $+2{,}1$\,pp AUROC L1 con avg Large$+$DermLIP & \textbf{Probado}, secci\'on~\ref{sec:dermapixel-ensemble} \\
Pérdida focal / re-ponderación de clases & Entrenamiento         & $\sim 5$ min Spark         & $+3{,}3$\,pp BAcc L1 y $+5{,}9$\,pp BAcc L2 sobre LP CE ($\gamma = 2{,}0$); sin efecto L3 & \textbf{Ejecutado} ($\gamma \in \{1,\,2,\,5\} \times \{\text{none},\,\text{balanced}\}$), secci\'on~\ref{sec:dermapixel-focal} \\
CBM sobre conceptos SkinCon              & Interpretable          & Anotación dermatológica    & Trazabilidad clínica                                          & Diseñado (Anexo~\ref{cap:anexof}) \\
Banco hospitalario HUSLL ($\sim 22$\,K) & Datos                  & $3$--$6$ meses (CEIC)      & Activación L3 directa (SpanDerm v1)                           & Bloqueado (revisión ética) \\
\hline
\end{tabular}
\end{table}
...
experimental del presente trabajo en la extensión del corpus vía
...
adaptación LoRA SpanDerm v0, \emph{Test-Time Augmentation},
...
secci\'on~\ref{sec:ft-results}, ensemble PanDerm$+$DermLIP en la
línea documentada en la secci\'on~\ref{sec:ensemble-results} y
...
variante directa SpanDerm v1), las cifras esperables para un
...
secci\'on~\ref{sec:ft-results} para PanDerm Base ajustado sobre
...
\subsection{SpanDerm: modelo dermatológico clínico en español}
\label{sec:linea-spanderm}

El dataset DermapixelAI~1.0 habilita el desarrollo de un proyecto
de modelos fundacionales dermatológicos clínicos en español,
denominado SpanDerm. El proyecto se articula en dos ramas
complementarias.

La primera es la rama \textbf{supervisada}: cabeza clasificadora
sobre encoder PanDerm Large adaptado mediante \emph{Low-Rank
Adaptation}. Comprende SpanDerm v0 ---cabeza L2 con LoRA $r=16$
ejecutada y reportada en la
secci\'on~\ref{sec:dermapixel-spanderm-v0}--- y SpanDerm v1
---cabeza L3 directa, condicionada al banco hospitalario HUSLL---,
descritas a continuación en los párrafos correspondientes.

La segunda es la rama \textbf{zero-shot multimodal}:
SpanDerm-CLIP, sistema dual-encoder estilo CLIP que alinea el
encoder visual PanDerm Large con un encoder textual multilingüe
(\texttt{intfloat/multilingual-e5-base}) sobre el mismo corpus
DermapixelAI~1.0. Se han ejecutado cinco iteraciones
experimentales (v1 a v5) reportadas íntegramente en la
secci\'on~\ref{sec:spanderm-clip}. La iteración v4
...
corpus con $15\,128$ imágenes externas genéricas, reporta una
...
discursivo castellano. Las contribuciones empíricas
complementarias son la recuperación cross-modal castellano-imagen
$\mathbf{14{,}0\times}$ lift sobre baseline aleatorio (v2,
$i2t$ R@$5 = 0{,}654$ frente a $0{,}047$ en régimen de
\emph{caption} rica, \emph{pool} $= 107$), el
diagnóstico mecanístico del techo del encoder visual congelado
via \emph{Linear Probing} y la reducción del ECE L3 del
$97\%$ ($0{,}7262 \to 0{,}0213$ con $\tau = 0{,}4165$ sobre v4)
mediante \emph{temperature scaling}~\cite{guo2017}. Una ablación
...
alineación cross-modal castellà, no el codificador visual ni la
...
La ejecución del protocolo (secci\'on~\ref{sec:dermapixel-spanderm-v0}
...
secci\'on~\ref{sec:sae-derm7pt}; el cuarto se ha ejecutado y se
reporta en la secci\'on~\ref{sec:sae-d7pt-e4} dentro de la misma
...
secci\'on~\ref{sec:sae-derm7pt}, identifica AUROC efectivo
...
secci\'on~\ref{sec:sae-derm7pt}) cuantifica el compromiso
...
secci\'on~\ref{sec:sae-derm7pt} documenta que $11$ de
...
interpretabilidad sin entrenamiento específico sobre el corpus en
...
secci\'on~\ref{sec:sae-d7pt-e4}: con sólo $0{,}56$\,M de
...
La trilogía experimental SpanDerm-CLIP reportada en
secci\'on~\ref{sec:spanderm-clip} delimita con precisión la palanca
...
arquitectónica idéntica y ampliación del corpus de
...
entrenamiento, $149$ sin imágenes en el corpus actual) no se
rellenan tras la incorporación del corpus externo.
...
lugar de ampliar el corpus de entrenamiento de forma
...
secci\'on~\ref{sec:linea-husll}) constituye el recurso natural
...
supervisado directo L3 (SpanDerm v1) cuando se supere el umbral
operativo de $\sim 30$ imágenes por clase L3
(secci\'on~\ref{sec:linea-spanderm}). La consecución del umbral
operativo $0{,}25$ por v4 sobre el corpus actual hace que esta
...
para SpanDerm-CLIP no es escala genérica de datos sino anotación
...
La incorporación del banco hospitalario habilita SpanDerm v1
(secci\'on~\ref{sec:linea-spanderm}), la validación clínica prospectiva
sobre cohorte real (secci\'on~\ref{sec:linea-prospectivo}) y un análisis
...
en secci\'on~\ref{sec:ensemble-results} es prueba de concepto sobre
...
(secci\'on~\ref{sec:auditoria}) es directamente extensible a los demás
...
evaluación, corpus de preentrenamiento), acompañado del script
...
(secci\'on~\ref{sec:dermapixel-m4} del Anexo~\ref{cap:anexoh}) con
...
recuperación visual densa, LLM multimodal en local con MedGemma)
habilita la construcción de un agente clínico orquestado bajo
control del facultativo. El esquema previsto contempla
herramientas externas auditables (acceso a literatura científica,
verificación de criterios clínicos contra guías de práctica,
consulta de la historia del paciente), invocadas por un LLM bajo
prompt clínico estandarizado y con trazabilidad de cada paso de
razonamiento. La diferencia operativa respecto al uso directo de
un LLM multimodal generalista
(secci\'on~\ref{sec:disc-llm}) es la inclusión de evidencia
estructurada externa al modelo, lo que reduce el sesgo léxico
documentado y facilita la justificación clínica de la salida. El
desarrollo es condicional a la disponibilidad de pesos abiertos
con capacidad de razonamiento estructurado (MedGemma~27B y
sucesores).
...
(secci\'on~\ref{sec:disc-zs}) sugiere que un reentrenamiento dirigido del
encoder textual de DermLIP sobre texto clínico en español aportaría
mejoras significativas sobre el rendimiento zero-shot en el ámbito
...
afirmaciones cualitativas defendibles (secci\'on~\ref{sec:respuesta}),
nueve contribuciones reproducibles (secci\'on~\ref{sec:contribuciones}),
cinco hallazgos metodológicos transferibles
(secci\'on~\ref{sec:hallazgos-conc}) y cuatro aportaciones a la práctica
clínica (secci\'on~\ref{sec:aportaciones-clinicas}).

Las ocho líneas de investigación abiertas
(secci\'on~\ref{sec:lineas-abiertas}) constituyen propuestas concretas con
...
La aportación intelectual del trabajo se sitúa en la conjunción
del rigor metodológico del benchmark cuantitativo, la construcción
de recursos abiertos para la comunidad y la formulación explícita
de líneas de continuación con base técnica y clínica
verificable. El siguiente paso natural es la primera publicación
derivada del trabajo, centrada en la comparativa de modelos
fundacionales bajo la ontología armonizada, seguida del
desarrollo de SpanDerm como modelo dermatológico para texto
clínico en español a partir del dataset DermapixelAI~1.0
extendido con el banco hospitalario del HUSLL.
