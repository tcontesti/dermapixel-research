...
\paragraph{Linear probe sobre el \emph{split} fijo.}
Se aplica el protocolo de linear probe (sección~\ref{sec:lp}) sobre
las $1\,062$ imágenes evaluables (\texttt{label\_source=ontology} y
modalidad clínica o dermatoscópica; train $N = 874$, validación
$N = 152$, test $N = 36$), con PanDerm Base
($768$-D) y Large ($1\,024$-D) como extractores congelados y head
...
no de revisión individual, se compara el linear probe sobre el
...
ablaciones adicionales sobre los mismos \emph{embeddings} (heads
...
Para evaluar si las representaciones de distintos encoders son
complementarias, se entrena una head logística por encoder
...
Large$+$DermLIP, máximo por clase y apilado con meta-head). La
...
Ningún encoder lidera simultáneamente L1, L2 y L3. En L1 lidera la
...
PanDerm Large individual, donde la long tail limita la señal
complementaria.
...
Este protocolo replica sobre DermapixelAI~1.0 el fine-tuning
...
parámetros entrenables, $8{,}3\%$ del encoder) y head
...
El fine-tuning denso parcial mejora al linear probe en L1 y L2, pero
...
conjunto, ni la combinación de encoders ni el fine-tuning
...
único encoder \emph{ganador} resulta una simplificación excesiva
...
\textit{Tercero}, la comparación con el linear probe supervisado
...
al mejor linear probe supervisado (PanDerm Large, $0{,}793$) dentro de
...
adicional; el grano grueso L1 y la long tail L3 sí requieren, en
cambio, clasificador supervisado. La síntesis tabular de estas
brechas ---penalización por idioma y distancia al linear probe
...
cuantificar la penalización por idioma sobre la long tail.
...
(tabla~\ref{tab:dermapixel-cv}), el linear probe sobre el \emph{split}
...
parámetros frente a los $25{,}2$\,M del fine-tuning parcial denso
...
tanto, al fine-tuning parcial denso en este régimen de datos.

\subsection{Limitaciones en L3}

El rendimiento cae notablemente en L3 (ranking por prototipos coseno,
Acc@1 $= 0{,}167$, $\times 42$ sobre el azar), donde $178$ de las $224$
clases L3 presentes en \emph{train} disponen de cinco o menos
imágenes: la principal
...
(\ref{cap:repo}, sección~\ref{sec:repo-ablaciones}); la head L3 directa queda
...
Las iteraciones v1$\to$v3, que actúan sobre el encoder textual y
...
encoder visual (LoRA $r=32$ sobre doce bloques frente a $r=16$ sobre
...
sobre la que se calibra \emph{H6}. La long tail tampoco se rellena: de
las $327$ clases L3 con $\leq 5$ imágenes, ninguna recibe imágenes
externas (las ocho clases que sí se refuerzan ya tenían entre $7$ y
$20$ imágenes base, es decir, quedan fuera de la cola).
...
sensibilidad de DermLIP al tokenizer
...
exactitud ---inferior a la head supervisada cerrada--- sino aceptar
...
La figura~\ref{fig:retrieval-es} muestra esta recuperación de forma
concreta: a partir de una consulta de texto libre en castellano, el
sistema devuelve los casos visualmente más parecidos del archivo.
...
última (exantema de sarampión) es un fallo honesto (rango $15$).}
...
evalúan encoders de dominio (PanDerm Base/Large, DermLIP v2),
vision-language médicos (BiomedCLIP, SigLIP-SO400M) y las dos ramas de
Dermapixel R0 sobre DermapixelAI~1.0. El repositorio reproducible
(\ref{cap:repo}, sección~\ref{sec:repo-ablaciones}) añade
dos ablaciones complementarias ---encoders generalistas no
médicos (DINOv2, CLIP-L) bajo linear probe y dos arquitecturas de
...
como estimación defendible del linear probe congelado. La elección de
