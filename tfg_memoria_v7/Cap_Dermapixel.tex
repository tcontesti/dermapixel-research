...
LogisticRegression. La evaluación L2 usa las $38$ subcategorías
...
Dado que el test fijo $N = 36$ es un único split, se estima el
rendimiento esperado mediante validación cruzada de cinco folds
agrupada por \texttt{case\_id} (StratifiedGroupKFold por L1
sobre las $1\,062$ imágenes), sin que ninguna imagen del mismo caso
caiga a la vez en train y test. La
...
sobre los embeddings congelados y se contrastan frente a cinco
...
fully-connected, con entropía cruzada reponderada por el
...
La evaluación zero-shot sobre DermapixelAI~1.0 arroja tres
...
\textit{Segundo}, traducir el prompt del inglés al castellano
...
entrenado y uno zero-shot: $+29{,}4$\,pp BAcc en L1 y
...
sección: DermLIP v2 zero-shot en inglés (AUROC $0{,}794$) iguala
...
caveats son específicos del régimen zero-shot: el
ensemble de tres plantillas por nivel es una formulación
mínima de prompt engineering ---la literatura emplea entre
...
clases L3 presentes en train disponen de cinco o menos
...
hiperparámetros, las particiones case-aware, la comparación frente a un
wrapper inglés con traducción ES--EN y la generación de captions están
...
$101$ imágenes de test bajo régimen híbrido H6 (logits visuales
y textuales con pesos $w_v = 0{,}3$, $w_t = 0{,}7$ calibrados sobre
validación), uniforme a las cinco iteraciones para la comparativa
cross-checkpoint. Los niveles L1 y L2 zero-shot sólo se reportan
para v4 y v5.
...
las captions ricas, quedan empatadas ($0{,}178 \to 0{,}198$,
ruido experimental): ninguna intervención textual extrae información que
las features visuales no contengan ya (el diagnóstico de techo
Linear Probing figura en el repositorio reproducible,
\ref{cap:repo}, sección~\ref{sec:repo-ablaciones}).

\paragraph{(2) La adaptación visual es la principal fuente de mejora.}
El salto $+4{,}95$\,pp entre v3 y v4 ($0{,}198 \to 0{,}248$, idéntico
régimen H6) se atribuye a la ampliación de capacidad del
...
$0{,}2079$ ($-4{,}0$\,pp): la escala genérica no sustituye a tener datos
del propio dominio. Aunque la parte visual mejora por separado
(H1 $0{,}168 \to 0{,}238$), las captions externas no suenan como el
archivo real y estropean la alineación en castellano sobre la que se
calibra el modo híbrido H6. Y la long tail ni se toca: de las $327$
...
accuracy@1 L3 $= 0{,}287$ ($+3{,}96$\,pp sobre v4), de modo que
la palanca limitante es la escala de la alineación cross-modal y
...
entre versiones, en régimen rich-caption
...
con el lift sobre el azar figura en el repositorio reproducible
(\ref{cap:repo}, sección~\ref{sec:repo-ablaciones}).

La recuperación cross-modal alcanza su pico en v2: i2t R@$5 = 0{,}654$,
un lift de $\mathbf{14{,}0\times}$ sobre el azar, y desciende en
las iteraciones posteriores (v4 $12{,}4\times$, v5 $10{,}8\times$). El
dato relevante es
la tensión que revela: v4 es la mejor versión clasificando
($0{,}2475$ en L3) pero v2 es la mejor recuperando. Optimizar la
clasificación y optimizar la búsqueda semántica no son, por tanto, el
mismo problema: la LoRA visual (v4) y las captions externas (v5)
...
La calibración de los logits zero-shot de la rama contrastiva
por temperature scaling reduce el \emph{Expected Calibration
...
inferencia (retrieval $k$-NN con FAISS y zero-shot
...
ensemble entre modelos dermatológicos y generalistas para
