...
La rama lingüística de los modelos vision-language contrastivos
(DermLIP, BiomedCLIP) está preentrenada sobre dataset en inglés. La
sensibilidad documentada al tokenizer (sección~\ref{sec:disc-zs},
...
tokenizers con vocabulario clínico hispanohablante. El dataset
...
($43$ entradas), pero a nivel L3 cada dataset conserva su long tail propia y los contrastes directos requieren agregación
...
La auditoría descrita en sección~\ref{sec:auditoria-resultados} identifica un
...
(sección~\ref{sec:benchmark}) permanecen defendibles incluso bajo el
...
acompañamiento con intervalos de confianza al $95\%$ por
\emph{bootstrap} ---incluido el \emph{gap}~I-VI de cada encoder---
se reporta en el repositorio reproducible (\ref{cap:repo},
...
Los protocolos de linear probe, eficiencia en etiquetas y
evaluación de equidad por fototipo operan con una única semilla
(\texttt{random\_state}~$=42$); el protocolo de fine-tuning emplea
...
linear probe) son cualitativas y robustas, pero los umbrales
...
El test de DeLong~\cite{delong1988} se ha aplicado a los contrastes
pareados entre encoders sobre tres \emph{endpoints} binarios
centrales de detección de lesión maligna ---Fitzpatrick17k (malignidad),
HAM10000 (melanoma) y DermapixelAI~1.0 (malignidad)---, con corrección
de Holm dentro de cada \emph{endpoint} (sección~\ref{sec:equidad-delong};
tabla detallada en el repositorio reproducible, \ref{sec:repo-tablas}). El cierre estadístico se concentra, no
obstante, en Fitzpatrick17k, el único con potencia muestral suficiente
($226$ positivos); en HAM10000 ($70$) y DermapixelAI~1.0 ($58$ en
validación cruzada) ningún contraste alcanza significación, un patrón
compatible con la falta de potencia y no con la ausencia de efecto. El
acompañamiento de cada cifra principal con su intervalo de confianza
al $95\%$ por \emph{bootstrap} se reporta en el repositorio
reproducible (sección~\ref{sec:repo-bootstrap-ci}); la extensión del
contraste pareado de DeLong al resto de tablas multiclase del capítulo
queda como continuación natural del trabajo.

La consecuencia operativa es que la superioridad de los encoders de
dominio sobre los generalistas queda establecida estadísticamente sobre
el \emph{endpoint} con potencia (Fitzpatrick17k), mientras que el resto
de hallazgos numéricos del capítulo~\ref{cap:resultados} ---y la
ordenación fina entre los dos líderes específicos--- deben interpretarse
en términos descriptivos y de magnitud del efecto: las afirmaciones de
\emph{superioridad estadísticamente significativa} en los demás
\emph{endpoints} requieren mayor tamaño muestral. Los patrones
cualitativos identificados en la discusión son, no obstante, robustos
frente a esta limitación por la magnitud absoluta de los efectos
cuantificados.
...
DermLIP sobre HAM10000 (sección~\ref{sec:zs-results}) cuantifica la
...
en el repositorio reproducible (\ref{cap:repo}, sección~\ref{sec:repo-ablaciones}) y en el documento formal de entrega
...
$32\%$ del dataset, lo que reproduce la long tail típica del
...
el repositorio reproducible (\ref{cap:repo}, sección~\ref{sec:repo-ablaciones}) es relevante para la interpretación
...
DermApIxel descrito en el repositorio reproducible (\ref{cap:repo}, sección~\ref{sec:repo-prototipo}) sobre cohorte del
...
Tres restricciones explícitas condicionan, además, el diseño
experimental y la interpretación de los resultados. En cuanto al
\textbf{hardware}, todas las ejecuciones se realizan sobre la NVIDIA
DGX Spark descrita en la sección~\ref{sec:infraestructura}, cuya
arquitectura aarch64 no coincide con el hardware x86\_64 dominante en
la literatura. En cuanto al \textbf{acceso a datos}, los conjuntos
internos del grupo de la Universidad Monash empleados en el
preentrenamiento de PanDerm no son accesibles para terceros
(sección~\ref{sec:lim-preentrenamiento}), lo que limita el trabajo a
los pesos liberados y a la evaluación sobre datasets externos
públicos. En cuanto al \textbf{acceso a modelos}, los pesos de
DermFM-Zero no son públicos en la fecha de cierre
(sección~\ref{sec:lim-pesos}), lo que impide su evaluación empírica
directa y restringe su tratamiento a la discusión conceptual.

Cinco elementos quedan, por decisión de alcance, fuera del trabajo,
y deben reconocerse como limitaciones del
...
no contemplado; y (v)~el análisis sistemático de calibración
(\emph{Brier score} y ECE en todas las tareas) y la evaluación de
utilidad clínica mediante análisis de curva de decisión
(\emph{decision curve analysis} y \emph{net benefit}), cuyo objeto
---fijar umbrales operativos y cuantificar el beneficio neto de una
decisión asistida--- excede el de este trabajo, centrado en la
capacidad discriminativa y la adaptación de los modelos; la
calibración se reporta de forma puntual donde resulta directamente
relevante (ECE en segmentación, en \emph{zero-shot} L3 y en el
\emph{endpoint} de melanoma de HAM10000), y su
tratamiento sistemático queda como continuación natural hacia una
eventual evaluación de despliegue.
...
evidencia empírica defendible del trabajo en relación con el
planteamiento descrito en sección~\ref{sec:objetivos}:

\begin{enumerate}
  \item La \textbf{ventaja del preentrenamiento sobre el dominio
  clínico} frente a encoders generalistas.
  \item La \textbf{saturación temprana del linear probe}, que alcanza
  el grueso de su rendimiento con $1$--$5\%$ de los datos etiquetados.
  \item La \textbf{brecha entre encoders específicos y LLMs
...
DermapixelAI~1.0 (sección~\ref{sec:lim-dermapixel}), su long tail en el
