...
L1/L2/L3, bajo cuatro protocolos principales (linear probe,
fine-tuning supervisado, segmentación binaria promptable y
...
$+3{,}6$\,pp en AUROC bajo linear probe, máxima sobre los datasets más
heterogéneos. El fine-tuning con la receta original aumenta la BAcc en
...
\paragraph{Segunda. Saturación temprana del linear probe.}
El linear probe sobre encoders preentrenados específicos de
...
subcategoría L2, long tail L3, ranking probabilístico, segmentación y
...
limitado por la long tail. El siguiente salto en rendimiento clínico no
se espera de sustituir el encoder, sino de ampliar y mejorar los
...
\item \textbf{Caracterización de la sensibilidad al tokenizer en
DermLIP.} Identificación del tokenizer y la formulación del
...
mediante fine-tuning del decoder.} Ajuste denso del
...
HAM10000 (M1) y SigLIP-Large SO400M con linear probe, que alcanza
...
DermapixelAI~1.0, articulado en una rama supervisada (head L2 con LoRA
...
El hallazgo transversal del trabajo es que ningún encoder ni
...
(clasificación general, subcategoría clínica L2, long tail L3,
...
\paragraph{Régimen de aplicabilidad del linear probe.}
La comparación cuantitativa sobre cuatro datasets
(sección~\ref{sec:disc-lp-vs-ft}) delimita un régimen operativo: para
datasets con $N_{\mathrm{train}} \lesssim 500$ imágenes, el linear probe sobre PanDerm Large supera al fine-tuning completo en las cuatro
métricas. El umbral se identifica sobre DDI ($N_{\mathrm{train}} = 427$),
donde el linear probe mantiene AUROC $0{,}827$ frente a $0{,}682$ del
fine-tuning, descenso diagnóstico de sobreajuste pese a las
...
fine-tuning si el ranking de probabilidades degrada.

\paragraph{Compatibilidad tokenizer-encoder en zero-shot.}
La AUROC en clasificación zero-shot con modelos vision-language
contrastivos depende críticamente de la compatibilidad entre el
tokenizer empleado para los \emph{prompts} y el espacio de
...
\paragraph{Ajuste del decoder sobre encoders promptables
preentrenados a gran escala.}
El fine-tuning del decoder de SAM2.1-Large con encoder
visual congelado (${\sim}4{,}2$\,M parámetros, $<2\%$ del total) supera
al fine-tuning completo de \emph{baselines} dermatológicos: las
...
long tail.}
Sobre el nivel L3, dominado por una long tail estricta, la recuperación
$k$-NN con FAISS sobre \emph{embeddings} congelados resulta competitiva
frente a las heads paramétricas, que carecen de muestras suficientes
...
head lineal multi-clase mediante regresión logística L-BFGS
($C = 1{,}0$, \texttt{max\_iter}~$=5\,000$) en datasets con
$N_{\mathrm{train}} \lesssim 500$, o head ajustada con la
receta de PanDerm Large en datasets con
$N_{\mathrm{train}} \gtrsim 5\,000$ y desbalance acusado.
Segmentación con SAM2.1-Large adaptado mediante fine-tuning
del decoder cuando la tarea requiera localización de lesión. Ensemble OR \emph{top-3}
de M1 (PanDerm Large \emph{fine-tuned}) y SigLIP-Large con linear probe cuando la tarea sea cribado oportunista de melanoma con
...
L1) aporta robustez frente a la incertidumbre en la long tail sin
...
académicos posteriores, la adaptación de modelos vision-language a
...
incorporación habilita la head L3 directa de Dermapixel R0
...
\paragraph{Aprendizaje activo \emph{in-domain} sobre la long tail L3.}
...
para la long tail L3 no es más datos genéricos, sino aprendizaje activo
...
Articulado en una rama supervisada (head L2 con LoRA, BAcc
...
con el banco hospitalario del HUSLL, que habilita la head L3 directa
($251$ diagnósticos), hoy limitada por la long tail de DermapixelAI~1.0.

\paragraph{Cobertura experimental sobre DermapixelAI~1.0.}
\label{sec:dermapixel-futuro}
Las técnicas de \emph{transfer learning} sobre el dataset ---fine-tuning
denso, adaptación LoRA de Dermapixel R0, \emph{Test-Time Augmentation},
ensemble multi-encoder y pérdida focal--- quedan ejecutadas y
...
Grupo~A y fine-tuning multitarea) están \emph{ejecutados}
...
hasta $0{,}926$) y el fine-tuning multitarea produce inferencia
...
La sensibilidad documentada al tokenizer (sección~\ref{sec:disc-zs})
...
lectura de conjunto no es la designación de un encoder vencedor,
...
arquitectura modular que combine encoders, niveles ontológicos
