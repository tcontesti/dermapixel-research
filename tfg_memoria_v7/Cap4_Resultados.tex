...
PAD-UFES-20 y DDI. Mejor valor por columna en negrita. Algunas filas
difieren ligeramente en el tamaño de test y en la convención de
promediado de la AUROC; el detalle celda a celda figura en el
...
datasets externos (``N/A'' indica no evaluado). La celda en rojo marca
el único solapamiento exacto: DermLIP v2 sobre Dermnet, contenido en
su preentrenamiento Derm1M (sección~\ref{sec:auditoria-resultados}).}
\label{fig:lp-per-dataset}
\end{figure}

PanDerm Large supera a las líneas base convolucionales en los tres
datasets, pero su ventaja no es uniforme: en dermatoscopia muy
estandarizada (HAM10000) los modelos generalistas la recortan
---SigLIP-Large logra incluso la mejor accuracy y AUROC---, mientras
que en los datasets clínicos más alejados del dominio web (PAD-UFES-20
y DDI) PanDerm encabeza con holgura. La figura~\ref{fig:lp-per-dataset}
extiende la comparación a los diez datasets y a DermLIP v2, BiomedCLIP
y DINOv2; la única cifra que debe leerse con cautela es la de DermLIP
v2 sobre Dermnet, contenido en su preentrenamiento
...
PubMedBERT al conjunto A3 de Derm1M ---la plantilla de \emph{prompts}
que obtuvo el mejor rendimiento en el trabajo original--- con
tokenizer GPT-2/CLIP eleva la AUROC de $0{,}366$ a $0{,}854$
($+48{,}8$\,pp). Los
...
trabajo (sección~\ref{sec:alcance}, elementos no abordados).
...
En conjunto, los tres \emph{endpoints} cuentan la misma historia, pero
con distinto grado de certeza estadística, y la diferencia está en
cuántos casos positivos aporta cada uno. Solo Fitzpatrick17k reúne los
suficientes ($226$) para que el test pueda separar a unos modelos de
otros: ahí la jerarquía «modelo específico de dominio $>$ modelo
generalista» resulta significativa, aunque sin llegar a distinguir entre
sí a los dos líderes específicos. Los otros dos \emph{endpoints}
---HAM10000, con $70$ melanomas, y DermapixelAI~1.0, con $58$ malignos---
reproducen el mismo patrón de forma descriptiva, con AUROC altas y muy
solapadas, pero con tan pocos positivos el test no llega a la
significación. Conviene subrayar la diferencia: eso indica falta de
potencia estadística ---no hay casos suficientes para confirmar la
diferencia---, no que la diferencia no exista. La conclusión sólida no
es, por tanto, coronar a un único modelo, sino constatar que toda una
\emph{familia} de encoders dermatológicos supera de forma consistente a
los generalistas, idea que se retoma en la discusión
