...
figura en el repositorio reproducible (\ref{cap:repo}, sección~\ref{sec:repo-reproducibilidad}).
...
que aflojarla degrada poco ($-2{,}8$\,pp al $+10\%$). La lectura
...
La consecuencia es interpretativa: las diferencias de $\sim 1$\,pp entre
arquitecturas (tabla~\ref{tab:seg-arquitecturas}) son menores que el
desacuerdo entre anotadores humanos, de modo que el resultado ya se
sitúa en el techo que la propia tarea permite. Dentro del alcance
evaluado ---segmentación binaria de lesión dermatoscópica con
\emph{prompt} de caja y test blindado de ISIC2018--- la principal
limitación reside ya en la incertidumbre de la anotación humana y no se
extiende a la segmentación multilesión ni al régimen sin
caja-\emph{prompt}. La supervisión en forma de lenguaje motiva el bloque
...
encoder. Esto se cuantifica sobre HAM10000 ($7$ clases,
$N_{\mathrm{test}} = 1\,232$) con tres configuraciones de \emph{prompts}
y dos tokenizadores distintos (cifras completas en el repositorio
reproducible, \ref{cap:repo}, sección~\ref{sec:repo-llm}).
...
que se mantiene en PAD-UFES-20 y DDI---; en el extremo inferior, BLIP-2 e
